## Пункт 17. Создание JavaScript-файла

Мы построили "скелет" игры с помощью `HTML`, "одели" его в стили `CSS`, но теперь пора вдохнуть в нее жизнь!
К сожалению, `HTML` и `CSS`, хоть и имеют возможности реализации какой-то логики (например, анимация), но возможности эти крайне примитивны. И хотя правила пятнашек, очевидно, нельзя назвать сложными, они нетривиальны и специфичны. То есть нужно их отдельно программировать. Но, как и было сказано ранее, `HTML` и `CSS` не являются языками программирования, то есть нужен другой инструмент. 

Точно так же, как для управления космолетом нужен бортовой компьютер, для управлением действиями на сайте нужен язык программирования `JavaScript`. Он, специально созданный для разработки web-решений (в том числе сайтов), крайне прост в освоении и использовании. `JS` поможет нам реализовать правила игры "Пятнашки".

В папке `js` создай файл `script.js`. В нем будет весь последующий код игры.

## Пункт 18. Инициализация всех необходимых переменных

Прежде чем начинать игру, нужно "собрать" все необходимые "ингредиенты" – объявить переменные. Если этого не сделать, они могут иметь случайные или пустые значения, что может привести к ошибкам.

Некоторые значения мы возьмем прямо из HTML-разметки. Например, если нам нужен текст из какого-то блока, мы можем "достать" его с помощью специального метода `getElementById`.

Вставь в `script.js` код:
```js
...
## Пункт 19. Функции начала игры

Теперь нам нужно, чтобы игра могла "стартовать" с правильной позиции. Главное, чтобы плитки оказались в случайном порядке – так интереснее! Мы создадим несколько функций для этого:
- создания полностью решенного состояния игры;
- инициализации игры;
- случайного перемешивания плиток;
- отрисовки поля;
...
## Пункт 20. Функции хода игры

Теперь требуется создать логику поведения игры во время ходов игрока. Для этого реализуем следующие функции (некоторые из них с закладыванием в будущее уже использовались в предыдущих функциях):
- обновления хода;
- получения соседей плитки;
- обработчика хода;
- проверки на то, решена ли головоломка.
...
## Пункт 21. Добавление слушателей

Наша игра готова к действию, но как она узнает, когда игрок совершает ход? И срабатывание ее действий ведь не поставить на таймер. Для решения данной проблемы нам нужны "слушатели событий" (`eventListener`). Они как радары, которые ждут определенного "сигнала" (события), например, клика мыши, и, когда он приходит, выполняют заданную функцию.
Мы добавим такие слушатели для тех элементов игры, с которыми игрок будет взаимодействовать.
...
## Пункт 22. Добавление слушателей для клавиш клавиатуры

Для удобства игроков мы добавим возможность управлять игрой не только мышью, но и клавиатурой. Это часто делает игру быстрее и приятнее!

И, конечно же, в конце мы запустим игру, чтобы она была готова к старту!
...
## Пункт 23. Подключение `JavaScript` к `HTML`

Теперь, когда у нас есть весь код игры, нужно "познакомить" наш HTML-файл с `JavaScript`. Для этого используется специальный тег `script`.

Вставь в `index.html` перед закрывающим тегом `body` после модального окна:
```html
<script src="js/script.js"></script>
```
Кстати, используя этот тег, можно было писать код прямо в HTML-файле, но по тем же соображениям, что и были вынесены в отдельный файл стили, был вынесен в отдельный файл и код игры.

> Переходи к выполнению инструкции в файле [`Instruction_7_Check_and_Publish.md`](/Instructions/Instruction_7_Check_and_Publish.md).